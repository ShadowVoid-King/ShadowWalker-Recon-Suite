...
recon_fast() {
    ...
    if command -v findomain &> /dev/null; then
        findomain -t "$domain" -u "$outdir/findomain.txt" 2>/dev/null || true
    fi
    
    # Process results
    if [[ -f "$outdir/subfinder.txt" ]] || [[ -f "$outdir/findomain.txt" ]]; then
        cat "$outdir"/*.txt 2>/dev/null | sort -u > "$outdir/subdomains.txt"
        log_success "Found $(wc -l < "$outdir/subdomains.txt") unique subdomains"
    fi
...
recon_deep() {
    ...
    # Run dnsgen if available
    if command -v dnsgen &> /dev/null; then
        log_info "Generating DNS permutations..."
        cat "$outdir/subdomains.txt" | dnsgen - | puredns resolve \
            --rate-limit-trusted "$RATE_LIMIT" 2>/dev/null > "$outdir/resolved.txt" || true
    fi
    
    # Run gobuster if available
    if command -v gobuster &> /dev/null; then
        log_info "Running gobuster DNS..."
        gobuster dns -d "$domain" -w "$GOBUSTER_WORDLIST" -t "$THREADS" \
            -o "$outdir/gobuster.txt" 2>/dev/null || true
    fi
    
    # Process all results
    cat "$outdir"/*.txt 2>/dev/null | sort -u > "$outdir/all_domains.txt"
    log_success "Found $(wc -l < "$outdir/all_domains.txt") total unique domains"
